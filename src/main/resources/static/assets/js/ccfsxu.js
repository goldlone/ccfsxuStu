...
var myUrl = "http://localhost:8080/ccfsxu";
var xhr = null;
var CODE_SUCCESS = 1001;
var CODE_FORMAT_ERROR = 2001;
var CODE_NOT_EXIST = 2002;
var CODE_EXCEPTION = 3001;
  ...
  if(isEmail($("#inputEmail").val())) {
    if(!isNull($("#inputPassword").val())) {
      $.ajax({
        ...
        success: function (res) {
          console.log(res);
          if(res.code==CODE_SUCCESS) {    // 登录成功，跳转
  ...
  if(xhr.readyState == 4 && xhr.status == 200) {
    ...
    if(obj.data.length!=0) {
      ...
      for(var i=0; i<obj.data.length; i++)
        str.append(obj.data.data[0]+"<br>");
      showAlertMessage("以下同学录入出错：<br>"+str);
    } else {
...
function uploadCSPScore() {
  ...
  var fileObj = $("#exampleInputFile")[0].files[0];
  var FileController = "/csp/addScoreFile";
  // FormData 对象
  var form = new FormData();
...
function addCert() {
  bootbox.confirm({
    ...
    callback: function (result) {
      ...
      var memberFee = $("input[name='memberFee']")[0].value;
      var notMemberFee = $("input[name='notMemberFee']")[0].value;
      console.log();
